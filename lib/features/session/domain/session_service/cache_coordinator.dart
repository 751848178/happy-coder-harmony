...
class _SessionServiceCacheCoordinator {
  ...
  Future<_CachedSessionRestoreResult> restoreCachedSessions() async {
    try {
      ...
      for (final cached in cachedSessions) {
        final localState =
            extractLocalSessionStateFromMetadata(cached.metadata);
  ...
  Session sessionFromCache(
    ...
    return Session(
      ...
      draft:
          _notifier._normalizeOptionalValue(localState?['draft']?.toString()),
  ...
  Map<String, dynamic>? extractLocalSessionStateFromMetadata(
    ...
    return _notifier._asStringMap(
        metadata[SessionServiceNotifier._localSessionSnapshotKey]);
  }
    ...
    try {
      ...
      for (final session in sessions) {
        ...
        localStateBySessionId[session.id] = buildLocalSessionSnapshot(
          session: session,
          loadedMessageCount:
              canPersistLoadedWindow ? cachedMessages!.totalMessageCount : null,
          loadedMessages:
    ...
    try {
      ...
      final totalMessageCount =
          restoreSessionMessageCountFromLocalSnapshot(localState) ??
              rawMessages.length;
      final messages = rawMessages.length >=
              _SessionServiceMessageCoordinator
          ...
          : restoreMessagesFromSnapshotPayload(
              rawMessages,
              maxMessages: SessionServiceNotifier
                  .sessionDetailAutomaticMessageWindowSize,
            );

      final nestedMessages = _notifier._nestSidechainMessages(messages);
      final resolvedMessages =
          _notifier._resolveHistoricalToolCallStatuses(nestedMessages);
