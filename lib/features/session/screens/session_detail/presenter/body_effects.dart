...
class _SessionScreenBodyEffectsState extends State<_SessionScreenBodyEffects> {
  ...
  void didUpdateWidget(covariant _SessionScreenBodyEffects oldWidget) {
    ...
    // Only schedule effect sync when properties that affect side-effects
    // actually changed.  This avoids scheduling unnecessary post-frame
    // callbacks on every rebuild triggered by unrelated state changes
    // (e.g. sending state, session metadata).
    if (!identical(oldWidget.bodyState, widget.bodyState) ||
        oldWidget.hasScrolledToLatest != widget.hasScrolledToLatest ||
        oldWidget.hasNewerMessages != widget.hasNewerMessages ||
        oldWidget.userHasScrolledUp != widget.userHasScrolledUp ||
        oldWidget.autoApproveEnabled != widget.autoApproveEnabled ||
        oldWidget.initialLoadComplete != widget.initialLoadComplete ||
        oldWidget.stickyTurnId != widget.stickyTurnId) {
      _scheduleEffectSync();
    }
  }
